...
## Use `setwd()` if needed
setwd("C:/Users/fobem/OneDrive/Desktop/Bellvue Data Science Course/dsc520/data/tidynomicon")
...
person_df2<-read.csv("person.csv",header=TRUE,sep=",",stringsAsFactors = FALSE)
head(person_df2)


## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <-read.csv("scores.csv",header=TRUE,sep=",",stringsAsFactors = FALSE)
head(scores_df)
...
setwd("C:/Users/fobem/OneDrive/Desktop/Bellvue Data Science Course/dsc520/data")

voter_turnout_df1 <- read_excel("G04ResultsDetail2004-11-02.xls",sheet = "Voter Turnout", skip = 1)
head(voter_turnout_df1)
...
person_df <-person_df <-dbGetQuery(db,"SELECT * FROM person")
