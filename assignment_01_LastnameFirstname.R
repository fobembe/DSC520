...
total_sleep_week1 <-sum(c(6.1,8.8,7.7,6.4,6.2,6.9,6.6))
...
total_sleep_week2 <-sum(c(7.1,7.4,7.9,6.5,8.1,8.2,8.9))
...
mean(c(6.1,8.8,7.7,6.4,6.2,6.9,6.6))
...
m <- matrix(c(student01,student02,student03),ncol=2)
print(m)
...
grades <- rbind(grades,c(95.2,94.1))
grades

## Add a new assignment column with `cbind()`
assignment04 <- c(92.1, 84.3, 75.1, 97.8)
grades <- cbind(grades,c(92.1,84.3,75.1,97.8))
grades
...
characters_df <- data.frame(name, race, in_fellowship, ring_bearer, age,stringsAsFactors =FALSE )
